...
st.markdown(
    ...
    <style>
        ...
        .section-label::before {
            background: linear-gradient(180deg, var(--gold), #ef4444);
            border-radius: 999px;
            content: "";
        ...
        div[data-baseweb="select"] > div:focus-within {
            ...
            box-shadow: 0 0 0 3px rgba(23, 59, 99, 0.12);
        }
        ...
        div[data-testid="stSelectSlider"] div[role="slider"] {
            ...
            box-shadow: 0 2px 8px rgba(242, 169, 0, 0.45) !important;
        }

        div[data-testid="stSlider"] [data-testid="stThumbValue"] {
            background: var(--navy) !important;
            border-radius: 8px !important;
            box-shadow: 0 3px 10px rgba(8, 35, 63, 0.18);
            color: #ffffff !important;
        }

        div[data-testid="stFormSubmitButton"] > button {
            background: linear-gradient(90deg, var(--gold), #ffc233);
            color: #111827;
            font-weight: 900;
            border: none;
            border-radius: 12px;
        ...
        div[data-testid="stFormSubmitButton"] > button:hover {
            background: linear-gradient(90deg, #f5b313, #ffd05a);
            color: #111827;
            border: none;
            box-shadow: 0 12px 24px rgba(242, 169, 0, 0.34);
...
with st.sidebar:
    ...
    st.write("Loan screening decision-support tool")
    ...
    st.write(f"Return cutoff: **{artifacts['return_threshold']:.1f}%**")
    ...
    if result is None:
        ...
        st.markdown(
            ...
            <div class="model-detail-card">
                <div class="model-detail-title">Decision thresholds</div>
                <div class="model-detail-row"><span>Approve</span><strong>Probability ≥ {artifacts['approve_probability_threshold']:.0%} and return ≥ {artifacts['return_threshold']:.1f}%</strong></div>
                <div class="model-detail-row"><span>Review</span><strong>Probability {artifacts['reject_probability_threshold']:.0%}–{artifacts['approve_probability_threshold']:.0%}, if return is nonnegative</strong></div>
                <div class="model-detail-row"><span>Reject</span><strong>Probability &lt; {artifacts['reject_probability_threshold']:.0%} or return &lt; {artifacts['return_threshold']:.1f}%</strong></div>
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            ...
            <div class="model-detail-card">
                ...
                <div class="model-detail-row"><span>Review</span><strong>Probability {artifacts['reject_probability_threshold']:.0%}–{artifacts['approve_probability_threshold']:.0%}, if return is nonnegative</strong></div>
                <div class="model-detail-row"><span>Reject</span><strong>Probability &lt; {artifacts['reject_probability_threshold']:.0%} or return &lt; {artifacts['return_threshold']:.1f}%</strong></div>
                <div class="model-detail-row"><span>Pessimistic return cutoff</span><strong>{artifacts['return_threshold']:.1f}%</strong></div>
            </div>
            """,
            unsafe_allow_html=True
        )
...
st.markdown(
    """
    <div style="margin-top: 28px; padding-top: 12px; border-top: 1px solid #d8dee8; color: #64748b; font-size: 13px; text-align: center; font-weight: 650;">
