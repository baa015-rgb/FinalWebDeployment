...
st.markdown(
    ...
    <style>
        ...
        div[data-testid="stFormSubmitButton"] > button {
            ...
            box-shadow: 0 10px 18px rgba(242, 169, 0, 0.22);
        }
...
with st.sidebar:
    ...
    st.write(f"Return threshold: **{artifacts['return_threshold']:.2f}%**")
...
if "selected_scenario" not in st.session_state:
    st.session_state["selected_scenario"] = "Review example"

if "evaluation_result" not in st.session_state:
    st.session_state["evaluation_result"] = None

st.markdown('<div class="scenario-row-title">Scenario preset</div>', unsafe_allow_html=True)
scenario_col_1, scenario_col_2, scenario_col_3 = st.columns(3)

with scenario_col_1:
    if st.button("Approve example", key="approve_scenario_button"):
        st.session_state["selected_scenario"] = "Approve example"
        st.session_state["evaluation_result"] = None

with scenario_col_2:
    if st.button("Review example", key="review_scenario_button"):
        st.session_state["selected_scenario"] = "Review example"
        st.session_state["evaluation_result"] = None

with scenario_col_3:
    if st.button("Reject example", key="reject_scenario_button"):
        st.session_state["selected_scenario"] = "Reject example"
        st.session_state["evaluation_result"] = None
...
st.markdown(
    f'<div class="active-scenario-note">Active preset: {scenario}. Adjusting fields creates a custom application.</div>',
    unsafe_allow_html=True
)
...
def decision_css_class(recommendation):
...
def model_details_html(installment):
    return f"""
    <div class="model-detail-card">
        ...
        <div class="model-detail-row"><span>Approve rule</span><strong>Repayment ≥ {artifacts['approve_probability_threshold']:.0%} and return ≥ {artifacts['return_threshold']:.2f}%</strong></div>
        <div class="model-detail-row"><span>Review rule</span><strong>Between approve and reject rules</strong></div>
        <div class="model-detail-row"><span>Reject rule</span><strong>Repayment &lt; {artifacts['reject_probability_threshold']:.0%} or return &lt; {artifacts['return_threshold']:.2f}%</strong></div>
        <div class="model-detail-row"><span>Output role</span><strong>Probability drives risk; pessimistic return checks downside value</strong></div>
    </div>
    """


def loan_summary_html(record, result):
    return f"""
    <div class="loan-summary-card">
        <div class="loan-summary-title">Loan summary</div>
        <div class="loan-summary-grid">
            <div><span>Amount</span><br><strong>${record['loan_amnt']:,.0f}</strong></div>
            <div><span>Term</span><br><strong>{record['term']}</strong></div>
            <div><span>Grade</span><br><strong>{record['grade']}</strong></div>
            <div><span>Purpose</span><br><strong>{format_choice(record['purpose'])}</strong></div>
            <div><span>FICO</span><br><strong>{int((record['fico_range_low'] + record['fico_range_high']) / 2)}</strong></div>
            <div><span>DTI</span><br><strong>{record['dti']:.1f}</strong></div>
        </div>
    </div>
    """
    ...
    if result is None:
        st.markdown(
            ...
            <div class="sticky-output">
                ...
                <div class="info-card">
                    Select a preset or adjust the application controls, then evaluate the loan.
                </div>
                {model_details_html(installment)}
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        result_installment = result["installment"]
        record = result["application_record"]

        st.markdown(
            f"""
            <div class="sticky-output">
                <div class="decision-card {decision_css_class(recommendation)}">
                ...
                {loan_summary_html(record, result)}
                {model_details_html(result_installment)}
            </div>
            """,
            unsafe_allow_html=True
        )
