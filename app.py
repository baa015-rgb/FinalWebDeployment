...
st.markdown(
    ...
    <style>
        ...
        /* Fix text contrast on Streamlit widgets */
...
def bounded_slider(label, min_value, max_value, value, step, key, value_format=None, left_label=None, right_label=None):
    # Streamlit only shows slider endpoints on hover. This wrapper prints the endpoints permanently.
    ...
    if left_label is None:
        left_label = str(min_value)

    if right_label is None:
        right_label = str(max_value)

    st.markdown(
        f'<div class="slider-bounds"><span>{left_label}</span><span>{right_label}</span></div>',
        unsafe_allow_html=True
    )
...
scenario = st.selectbox(
    "Scenario",
    ["Moderate-risk example", "Lower-risk example", "Higher-risk stress test", "Custom"],
    index=0,
    help="Use presets for demo scenarios or choose Custom."
)

st.markdown(
    '<div class="scenario-note">Preset scenarios are for demonstration; use Custom for a specific application.</div>',
    unsafe_allow_html=True
)
    ...
    with st.form("loan_input_form"):
        ...
        with st.container(border=True):
            ...
            with credit_col_1:
                ...
                credit_history_years = bounded_slider(
