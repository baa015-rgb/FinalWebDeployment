...
st.markdown(
    ...
    <style>
        ...
        /* Make opened dropdown menus readable on Render/Streamlit */
...
def input_slider(label, min_value, max_value, value, step, key, value_format=None):
    # Compact wrapper for sliders. Streamlit shows the current value directly on the control.
    return st.slider(
        label,
        min_value=min_value,
        max_value=max_value,
    ...
    if result is None:
        ...
        st.markdown(
            ...
            <div class="model-detail-card">
                ...
                <div class="model-detail-row"><span>Approval threshold</span><strong>{artifacts['approve_probability_threshold']:.0%}</strong></div>
                <div class="model-detail-row"><span>Reject threshold</span><strong>{artifacts['reject_probability_threshold']:.0%}</strong></div>
            </div>
            """,
            unsafe_allow_html=True
        )
